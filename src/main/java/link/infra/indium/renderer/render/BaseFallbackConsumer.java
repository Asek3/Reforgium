...
public class BaseFallbackConsumer extends BaseQuadRenderer implements BakedModelConsumer {
	private static final Value MATERIAL_FLAT = (Value) ReforgiumRenderer.INSTANCE.materialFinder().disableAo(0, true).find();
	private static final Value MATERIAL_SHADED = (Value) ReforgiumRenderer.INSTANCE.materialFinder().find();
